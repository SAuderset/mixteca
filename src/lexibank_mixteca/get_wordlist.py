...
# if your wordlist does not contain "segments" or "tokens"
    wl.add_entries("form", "tokens", lambda x: 0)

# make an array with language names
    doc = wordlist.cols

# doculect by doculect find the profile and entries and entries do the tokenization 
    for name in doc:
        prf = Profile.from_file(ds.dir.joinpath("etc", name+".tsv"))
        tmp_tokenizer = Tokenizer(profile=prf)
        for idx, doc, fm in wordlist.iter_rows('doculect', 'form'):
            if doc == name
                wl[idx, 'tokens'] = tmp_tokenizer(fm, column="IPA")
