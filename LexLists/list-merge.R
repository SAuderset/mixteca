...
m2 <- full_join(m1, joss, by = c("CONCEPTICON_ID", "ADD_CONCEPT")) %>% select(CONCEPTICON_ID, CONCEPTICON_GLOSS = CONCEPTICON_GLOSS.x, ADD_CONCEPT, ID_EWC:ID_MAT, ID_JOSS = NUMBER, SPANISH_EWC:SPANISH_MAT, SPANISH_JOSS = SPANISH, ENGLISH_EWC:ENGLISH_MAT, ENGLISH_JOSS = ENGLISH)
glimpse(m2)

# merge with durr
m3 <- full_join(m2, durr, by = c("CONCEPTICON_ID", "ADD_CONCEPT")) %>% select(CONCEPTICON_ID, CONCEPTICON_GLOSS = CONCEPTICON_GLOSS.x, ADD_CONCEPT, ID_EWC:ID_JOSS, ID_DURR = NUMBER, SPANISH_EWC:SPANISH_JOSS, ENGLISH_EWC:ENGLISH_JOSS, ENGLISH_DURR = ENGLISH)
glimpse(m3)

# merge with swanton-mendoza
m4 <- full_join(m3, swme, by = c("CONCEPTICON_ID", "ADD_CONCEPT")) %>% select(CONCEPTICON_ID, CONCEPTICON_GLOSS = CONCEPTICON_GLOSS.x, ADD_CONCEPT, ID_EWC:ID_DURR, ID_SWME = NUMBER, SPANISH_EWC:SPANISH_JOSS, SPANISH_SWME = SPANISH, ENGLISH_EWC:ENGLISH_DURR)
glimpse(m4)

# merge with padgett
m5 <- full_join(m4, pdt, by = c("CONCEPTICON_ID", "ADD_CONCEPT")) %>% select(CONCEPTICON_ID, CONCEPTICON_GLOSS = CONCEPTICON_GLOSS.x, ADD_CONCEPT, ID_EWC:ID_SWME, ID_PADG = NUMBER, SPANISH_EWC:SPANISH_SWME, SPANISH_PADG = SPANISH, ENGLISH_EWC:ENGLISH_DURR, ENGLISH_PADG = ENGLISH)
glimpse(m5)
...
table(m6$COVERAGE)
# approx. 333 concepts that are in more than 1 list
